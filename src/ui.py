...
class UI:
    ...
    def start(self):
        ...
        btn_huff_compress = tk.Button(
            master=window,
            text="compress",
            command=lambda: self.lbl_info.config(
                text=self.huff_compress(ent_file.get()))
        ...
        btn_huff_decompress = tk.Button(
            master=window,
            text="decompress",
            command=lambda: [self.huff_decompress(
                ent_file.get()), self.lbl_info.config(text="done decompressing huff")]
        ...
        btn_lz_compress = tk.Button(
            ...
            text="compress",
            command=lambda: self.lbl_info.config(
                text=self.lz_compress(ent_file.get()))

        )
        btn_lz_decompress = tk.Button(
            master=window,
            text="decompress",
            command=lambda: [self.lz_decompress(ent_file.get()),
                             self.lbl_info.config(text="done decompressing lz")]
